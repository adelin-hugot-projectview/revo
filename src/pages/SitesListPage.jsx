...
const SitesListPage = ({ sites, teams = [], colors, statusColumns = [], onSiteClick, onAddSite, onClientClick, onUpdateSiteStatus }) => {
    ...
    const filteredSites = useMemo(() => sites.filter(site => {
        ...
        const dateMatch = (!start || siteStartDate >= start) && (!end || siteEndDate <= end);
    ...
    return (
        <div className="h-full flex flex-col">
            <div className="flex justify-between items-center mb-4">
                <h1 className="text-3xl font-bold text-gray-800 font-['Poppins']">Chantiers</h1>
            ...
            <div className="flex flex-wrap items-center gap-4 mb-4 p-4 bg-gray-50 rounded-lg">
                <div className="relative">
                    ...
                    <input type="text" placeholder="Rechercher..." value={searchTerm} onChange={(e) => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg w-64 focus:outline-none focus:ring-1 focus:ring-green-600"/>
                </div>
                <select value={statusFilter} onChange={e => { setStatusFilter(e.target.value); setCurrentPage(1); }} className="p-2 border border-gray-300 rounded-lg text-sm bg-white focus:ring-green-600">
                    <option value="">Tous les statuts</option>
                    {statusColumns.map(s => <option key={s.id} value={s.id}>{s.name}</option>)}
                </select>
                <select value={teamFilter} onChange={e => { setTeamFilter(e.target.value); setCurrentPage(1); }} className="p-2 border border-gray-300 rounded-lg text-sm bg-white focus:ring-green-600">
                    <option value="">Toutes les équipes</option>
                    {teams.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}
                </select>
                <DateRangePicker onDateChange={(start, end) => { setDateRange({ start, end }); setCurrentPage(1); }} colors={colors} />
            </div>
            <div className="flex-grow bg-white rounded-xl shadow-sm overflow-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50 sticky top-0">
                        <tr>
                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nom du chantier</th>
                            ...
                            <tr key={site.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                     <div className="flex items-center gap-2">
                                        ...
                                        {site.clientData && (
                                            <button onClick={() => onClientClick(site.clientData)} className="p-1 rounded-full hover:bg-gray-200 text-gray-500 hover:text-gray-800 transition-colors" aria-label="Voir le client">
                                                <Info size={14} />
                                            </button>
                                        )}
                                    </div>
