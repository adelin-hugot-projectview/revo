                ...
                <div className={`flex-grow grid grid-cols-${days.length}`}>
                    {days.map((day) => (
                        <div key={day.toISOString()} className="relative border-l">
                            ...
                            {sites.filter(event => new Date(event.date).toDateString() === day.toDateString())
                                 .map(event => {
...
const CalendarPage = ({ sites, colors, onSiteClick }) => {
    ...
    const handleDayClick = (day) => {
        const sitesOnDay = sites.filter(site => new Date(site.date).toDateString() === day.toDateString());
