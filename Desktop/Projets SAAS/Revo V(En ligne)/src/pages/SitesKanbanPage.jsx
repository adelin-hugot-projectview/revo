    ...
    const moveSite = (draggedId, newColumnId, dragIndex, hoverIndex) => {
        setLocalSites((prevSites) => {
            ...
            if (draggedSite.status_id === newColumnId) {
                const sitesInColumn = prevSites.filter(s => s.status_id === newColumnId).sort((a, b) => a.kanban_position - b.kanban_position);
                ...
                const newSites = prevSites.map(s => {
                    if (s.id === draggedId) {
                        return { ...s, status_id: newColumnId, kanban_position: hoverIndex };
                    }
                    return s;
                });
                return newSites;
                ...
                <div className="flex-grow flex gap-6 overflow-x-auto pb-4">
                    {sortedStatusColumns.map(status => (
                        <KanbanColumn
                            key={status.id}
                            status={status}
                            sites={localSites.filter(site => site.status_id === status.id).sort((a, b) => a.kanban_position - b.kanban_position)}
                            onSiteClick={onSiteClick}
                            colors={colors}
                            moveSite={moveSite}
